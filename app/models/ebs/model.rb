...
class Ebs::Model < ActiveRecord::Base
  ...
  def create_or_update(*args, &block)
    if Ebs::Model.using_sqlserver? && self.respond_to?(:id) && [nil,0].include?(self.id)
      ...
      elsif self.class.find_by_sql "select top 1 'x' from sys.sequences where name = '#{self.class.sqlserver_sequence_name}' and schema_id = 1"
        next_id = self.class.find_by_sql "select NEXT VALUE FOR [dbo].[#{self.class.sqlserver_sequence_name}] AS id"
        self.id = next_id[0]['id']
      end
    end
    super
