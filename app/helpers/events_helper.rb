...
module EventsHelper
  ...
  def delete_event_button(event)
    link_to image_tag("events/delete_event.png"), person_event_url(event.person, event, :eventtype => (event.title == 'Outcome' ? event.title : nil) ),
  ...
  def create_event_form(klass,html_options = {},remote = false,&block)
    form_for(@topic.kind_of?(Person) ? @topic.send(klass.name.tableize).new : klass.new, :remote => remote,
      ...
      if @topic.kind_of? Person
        ...
        if @tutorgroup
          concat(select_tag :person_id, options_for_select(@topic.person_courses.where(:tutorgroup => @tutorgroup).map{|p| [p.person.name, "#{p.person.surname} #{p.person.forename}", p.person.mis_id]}.sort_by {|ary| ary[1]}), :prompt => "Select a Person")
        else
          concat(select_tag :person_id, options_for_select(@topic.people.map{|p| [p.name, "#{p.surname} #{p.forename}", p.mis_id]}.sort_by {|ary| ary[1]}), :prompt => "Select a Person")
        end
      end
