...
Leap finds the associated user in LEAP.PEOPLE (or creates one if there isn't one yet) and stores this in the Person.user variable. It also
stores an affiliation in Person.affiliation. It uses the first affiliation it finds out of this list (Shibboleth can be configured to pass multiple but
Leap only uses one).
