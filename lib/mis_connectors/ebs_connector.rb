...
module MisPerson
  ...
  module ClassMethods
    ...
    def import(mis_id, options = {})
      ...
      if (ep = (Ebs::Person.find_by_person_code((mis_id.to_s.match(/\d{3}/) ? mis_id.to_s.tr('^0-9', '') : mis_id)) ||
        ...
        if @person.new_record? || ep.updated_date.nil? || (@person.updated_at < ep.updated_date)
          @person.update_attributes(
            ...
            address: ep.address ? [ep.address.address_line_1, ep.address.address_line_2,
                                   ep.address.address_line_3, ep.address.address_line_4].reject(&:blank?) : [],
            ...
            username: (ep.send(Settings.ebs_username_field) || ep.id.to_s),
            personal_email: ep.personal_email,
            home_phone: ep.address && ep.address.telephone,
            note: (ep.note && ep.note.notes) ? (ep.note.notes + "\nLast updated by #{ep.note.updated_by || ep.note.created_by} on #{ep.note.updated_date || ep.note.created_date}") : nil
          )
  ...
  def timetable_events(options = {})
    reds = if options == :next
             Ebs::RegisterEventDetailsSlot.where(object_id: mis_id, object_type: %w(L T))
             ...
             Ebs::RegisterEventDetailsSlot.where(object_id: mis_id, object_type: %w(L T), planned_start_date: from..to)
           end
    reds.map do |s|
      ...
      t.mark            = s.usage_code
      t.status          = s.status
      t.rooms           = s.rooms.map(&:room_code)
      t.teachers        = s.teachers.map { |x| x.try(:name) }
      t
    end
  end
  ...
  def import_attendances
    if [Settings.attendance_table, Settings.attendance_week_column, Settings.attendance_culm_column, Settings.attendance_date_column].detect(&:blank?)
      logger.error "Attendance table not configured"
      return false
    end
    last_date = if attendances.empty?
                  Date.civil(1900, 1, 1)
                else
