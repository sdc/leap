...
# =end
  ...
  def import_quals
    ...
    mis_person.attainments.where("NOT (upper(description) LIKE '%%NOT APPLICABLE%%' and source = 'PLR data') and upper(grade) NOT LIKE 'NA%%' and upper(grade) NOT LIKE 'OTH%%' and upper(grade) NOT LIKE '%%UNCLASSIFIED%%' and upper(grade) NOT LIKE '%%NO SHOW%%' and upper(grade) NOT LIKE '%%P ALPHA-NUMERIC VALUE%%' and upper(grade) != '01' and dbo.RegExIsMatch(ltrim(rtrim(fes_qualification_aim)), '^[0-9]') = 1").each do |at|
